...
import '../ViewModel/Controller/AdminController/CourseAllocationController.dart';

class CourseAllocationScreen extends StatefulWidget {
  final String? token;
  const CourseAllocationScreen({super.key, this.token});
...
class _CourseAllocationScreenState extends State<CourseAllocationScreen> {
  final CourseAllocationController controller = Get.put(CourseAllocationController());

  @override
  void initState() {
    super.initState();
    // ✨ FIX: Screen load hotay hi Controller ko Token assign kar diya gaya hai
                    ...
                    children: [
                          ...
                          child: Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    Expanded(child: _buildGlassDropdown("Department", controller.departments, controller.selectedDept)),
                                    const SizedBox(width: 12),
                                ...
                                Row(
                                  children: [
                                    ...
                                    const SizedBox(width: 12),
                                    Expanded(child: _buildGlassDropdown("Batch", controller.batches, controller.selectedBatch)),
                                  ],
                                ),
                                const SizedBox(height: 12),
                                _buildGlassDropdown("Section", controller.sections, controller.selectedSec),
                              ],
                            ),
                          ),
                        ),
                      ...
                      LayoutBuilder(
                        builder: (context, constraints) {
                          // Allocates safe horizontal expansion layout for grid columns
                              ...
                              child: Column(
                                children: [
                                  ...
                                  Container(
                                    ...
                                    child: const Row(
                                      children: [
                                        Expanded(flex: 2, child: Text("Batch", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13))),
                                        ...
                                        SizedBox(height: 20, child: VerticalDivider(color: Colors.white30, width: 10)),
                                        Expanded(flex: 3, child: Text("Course", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13))),
                                        SizedBox(height: 20, child: VerticalDivider(color: Colors.white30, width: 10)),
                                        Expanded(flex: 2, child: Text("Teacher Name", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13))),
                                        SizedBox(height: 20, child: VerticalDivider(color: Colors.white30, width: 10)),
                                  ...
                                  Container(
                                    ...
                                    child: Obx(() {
                                      ...
                                      if (controller.filteredAllocations.isEmpty) {
                                        return const Center(
                                          child: Text(
                                      ...
                                      return ListView.separated(
                                        ...
                                        itemBuilder: (ctx, i) {
                                          var record = controller.filteredAllocations[i];

                                          return Container(
                                            padding: const EdgeInsets.symmetric(vertical: 10, horizontal: 12),
                                            child: Row(
                                              children: [
                                                Expanded(flex: 2, child: Text(record['batch']?['batch_name']?.toString() ?? '-', style: const TextStyle(color: Colors.black87, fontWeight: FontWeight.w600, fontSize: 13), maxLines: 1, overflow: TextOverflow.ellipsis)),
                                                ...
                                                const SizedBox(height: 24, child: VerticalDivider(color: Colors.black12, width: 10)),
                                                Expanded(flex: 2, child: Text(record['teacher']?['teacher_name']?.toString() ?? '-', style: const TextStyle(color: Colors.black54, fontSize: 13), maxLines: 1, overflow: TextOverflow.ellipsis)),
                                                const SizedBox(height: 24, child: VerticalDivider(color: Colors.black12, width: 10)),
                                                SizedBox(
                                                  width: 80,
                                                  child: Row(
                                                    ...
                                                    children: [
                                                      ...
                                                      IconButton(
                                                        ...
                                                        onPressed: () => controller.deleteAllocation(record['id']),
                                                      ),
                                                    ],
                                                  ),
                                                )
  ...
  Widget _buildGlassDropdown(String label, RxList<String> optionsList, RxString targetingValue) {
    return Obx(() {
      ...
      // Crash-Guard Validation: ensures runtime value synchronizes with dataset arrays safely
      bool isValidValue = optionsList.contains(currentValue);
      if (!isValidValue && currentValue.isNotEmpty) {
        // Safe context state resetting without firing during build pipeline
      ...
      return DropdownButtonFormField<String>(
        ...
        onChanged: (newValue) {
          if (newValue != null) {
            ...
            // Trigger cascading filter safely if Department is selected
            if (label == "Department") {
              controller.handleDepartmentChange(newValue);
            } else {
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () {
                ...
                controller.updateAllocation(record['id'], body);
